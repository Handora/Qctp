...
serveGet(int connfd, SSL *CTX, char *filename, struct stat* sbuf, int close) {
  ...
  err = getStatic(connfd, CTX, filename, sbuf, close);

  if (err == -2) {
    sprintf(errorMessage, "error in writen2: %s", strerror(errno));
...
int
getStatic(int connfd, SSL *CTX, char *filename, struct stat *sbuf, int closes) {
